...
State = tuple[tuple[int, int], Direction, int, list[tuple[int, int]]]
...
def main(inp: str) -> None:
    ...
    q: deque[State] = deque()
    q.append(((py, px), ">", 0, [(py, px)]))
    best_tiles: set[tuple[int, int]] = set()
    best_score = 2**63 - 1
    while len(q) > 0:
        state = q.popleft()
        (y, x), direction, score, tiles = state
        if score > best_score:
            continue
        if grid[y][x] == "E":
            if score == best_score:
        ...
        if 0 <= ny < size_y and 0 <= nx < size_x and grid[ny][nx] != "#":
            q.append(((ny, nx), direction, score + 1, [*tiles, (ny, nx)]))

        q.append(
            ((y, x), rotate(direction, 1), score + 1000, tiles),
        )
        q.append(((y, x), rotate(direction, -1), score + 1000, tiles))
